...
\ansline{(Settled at problem 9: no --- both rooms are 9th grade; not one senior was asked.)}
\end{hookbox}
...
Frame: \labelbox{3.0cm}{sampling frame}\quad Left: \labelbox{2.1cm}{convenience}\quad Right: \labelbox{2.1cm}{probability}\par}
\\
& \notesprompt{Who chose each sample --- chance, or convenience? Name someone who could never have been picked.}
\begin{probgrid}{|Y|Y|}
\pcell{1}{A pool staff member asks the $75$ members who arrive on Tuesday evening. Then: all $900$ students are numbered and a computer picks $60$. Who chose each?}{1.6cm}{Convenience (the schedule chose), then chance (nobody left out).} & \pcell{2}{A teacher surveys her own third-period class --- she picked no favorites. \textbf{Vote first, then decide.}}{1.6cm}{Convenience. A fourth-period student could never be picked.} \\ \hline
\end{probgrid}
\\ \hline
...
\end{tikzpicture}\par\vspace{4pt}
Top: \labelbox{2.6cm}{simple random}\quad Bottom: \labelbox{2.2cm}{systematic}\quad $951$: \labelbox{2.0cm}{out of range}\par}
\\
& \notesprompt{Run both techniques on Riverbend.}
\begin{probgrid}{|Y|Y|}
\pcell{3}{Students are numbered $001$--$900$. What do you do with $951$ --- \emph{skip} it, or \emph{stop}? What does the generator do next?}{1.4cm}{Skip it --- $951$ is out of range; $366$ is next.} & \pcell{4}{Systematic: $k = 900 \div 60 = {}$? The random start lands on $7$. Write the next three names taken --- and circle the one number chance chose.}{1.4cm}{$k = 15$; then 22, 37, 52. Only the start, 7.} \\ \hline
\end{probgrid}
\\ \hline
...
\end{tikzpicture}\par\vspace{4pt}
Each grade is one \labelbox{2cm}{stratum}\quad it reaches \labelbox{1.5cm}{all} grades\par}
\\
& \notesprompt{Finish the allocation the warm-up started.}
\begin{probgrid}{|Y|Y|}
\pcell{5}{The warm-up found $270 \div 15 = 18$ ninth graders. Compute the other three shares --- then add all four. Do they total $60$?}{1.6cm}{16, 14, 12. \par $18 + 16 + 14 + 12 = 60$ --- yes.} & \pcell{6}{Stratifying \emph{guarantees} every grade is heard; a fair SRS of $60$ names only makes it \emph{likely}. Why?}{1.6cm}{An SRS might draw few or no seniors; stratifying draws 12 every time.} \\ \hline
\end{probgrid}
\\ \hline
...
\writespace{1.0cm}{Homerooms are grouped by grade --- a room is alike inside --- so two whole rooms can both be 9th grade and hold no seniors.}
\\
& \notesprompt{Now settle the hook vote.}
\begin{probgrid}{|Y|Y|}
\pcell{7}{Maya drew rooms $4$ and $7$ and surveyed everyone in both. How many students is that, and which technique is it?}{1.3cm}{$2 \times 30 = 60$ --- a cluster sample.} & \pcell{8}{Now read the map. Which grade is room $4$? Room $7$? How many seniors are in Maya's $60$?}{1.3cm}{Both are 9th grade. Seniors in Maya's 60: zero.} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{9}{\textbf{Vote again:} yes \; no \; it depends. Did chance choose? Did anyone count wrong? Then what went wrong?}{1.7cm}{No. Chance chose and nobody miscounted --- but both rooms are 9th.} & \pcell{10}{Is a Riverbend homeroom a stratum or a cluster? Which technique matches groups like that, and what does it take from each?}{1.7cm}{A stratum --- alike inside. Stratified takes some from every grade.} \\ \hline
\end{probgrid}
\\ \hline
...
\begin{probgrid}{|Y|Y|}
\pcell{11}{Name the technique in each plan: (i)~a computer draws $60$ of the numbered $600$; (ii)~$3$ buildings from a hat, all $20$ in each; (iii)~a random sample from each unit type, in proportion; (iv)~every $10$th name on the resident list after a random start.}{1.8cm}{(i) SRS; (ii) cluster; (iii) stratified; (iv) systematic.} & \pcell{12}{The stratified plan: $60$ of $600$ is $1/10$. Compute all three shares and check the total.}{1.8cm}{Studios 12, one-bed 28, two-bed 20. \par $12 + 28 + 20 = 60$.} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{13}{The hat gives buildings $2$, $5$, and $6$. Who is in the sample? Is a building a mix of the complex, or a stratum? What did the draw get wrong, even though chance chose?}{1.7cm}{All 60 studio households: a building is one unit type, a stratum.} & \pcell{14}{Which plan should management run --- \emph{because the groups are}\,\dots? And with only a list of \emph{buildings}, never of residents, which technique still runs?}{1.7cm}{Stratified --- unit types differ on cars. Else cluster.} \\ \hline
\end{probgrid}
\\ \hline
\end{guidednotes}
