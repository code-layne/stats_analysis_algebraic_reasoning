...
% (2026-09-12 shape): I do / we do in rows 1-4 (problems 1-10) -> we do the Guided Practice row
% (problems 11-14). THERE IS NO INDEPENDENT PRACTICE SET IN THE NOTES — the homework is the individual
...
\begin{tcolorbox}[colback=frost, colframe=cerulean, boxrule=0.9pt, arc=2mm,
    ...
    rows, problems 1--10: you read the definition, mark up the display, and work the first
    problem of each grid; the class works the rest); the Guided Practice row (problems 11--14)
    ...
    by grade ($1$--$9$ ninth, $10$--$17$ tenth, $18$--$24$ eleventh, $25$--$30$ twelfth). It is
    printed on the row~4 display, so \textbf{keep the notes turned to row~3 until you get
    there} --- the hook and problems 1--7 run on the hat draw alone, and problem~8 is where
    the class reads the map.
\end{tcolorbox}
...
Warm-Up & 5 & Riverbend's $900$: a percent, that percent of $60$, an interval down a list ---
  ...
  and leave it up --- row~2 names it \\
Guided Notes \& Practice & 35 & Fill the vocabulary box as each term is named; work rows
  1--4 of the table (problems 1--10) with the pen in their hand; then the Guided Practice row,
  \emph{Cedar Ridge} (11--14), together & \textbf{I do / we do, row by row}: read the
  definition, mark up the display, work problems 1, 3, 5, 7; the class works the rest (20)
  $\to$ \textbf{we do} Guided Practice 11--14 (15) \\
...
\begin{minipage}[t]{\linewidth}\vspace{0pt}
    ...
    \begin{itemize}[leftmargin=1.1em, itemsep=1pt, topsep=2pt]
      \item \textbf{1.} $270$ of $900$ as a percent. \textbf{Spirals} to Lesson 1.1's relative
            frequency; it is the share row~3 (\emph{Stratified}) preserves in problems 5--6.
      \item \textbf{2.} $30\%$ of $60$. \textbf{Seeds:} the stratified allocation with the
            name removed --- problem~5 opens on this $18$ and asks what they were doing.
      \item \textbf{3.} $900 \div 60 = 15$, start $7$, then $22$ and $37$. \textbf{Seeds:} the
            systematic sample with the name removed --- row~2 and problem~4 attach the name to
...
\begin{minipage}[t]{\linewidth}\vspace{0pt}
    ...
    \begin{itemize}[leftmargin=1.1em, itemsep=1pt, topsep=2pt]
      \item \textbf{Debrief item~3 aloud} and write $900 \div 60 = 15$; $7$, $22$, $37$ on the
            board. Leave it up all period; row~2 hangs \emph{systematic sample} on that line.
...
mention that homerooms are grouped by grade.} Say only that the answer is problem~9, in the
\emph{Cluster} row, and that they will vote again there. The vote is what makes the crux land: students
...
The notes are \textbf{one Main Ideas / Notes table}: four instruction rows (problems 1--10),
then the Guided Practice row (11--14). \textbf{The rhythm in every row is the same}: read the
...
\emph{as each term is named}, never in advance. There are no fill-in blanks this lesson ---
every answer is written in a problem's own space, or on a \texttt{labelbox} under a display.
\begin{multicols}{2}
\textbf{I do / we do, row by row (about 20 min).}

\emph{Row 1, Who Chooses? (problems 1--2).} Read the frame definition and name the two panels
of the display --- the office list is the \emph{sampling frame}, left is \emph{convenience},
right is a \emph{probability} sample. Work problem~1 yourself, both halves of it.
\textbf{Then take problem~2 and a hand count before anyone writes}: half the room calls
the third-period class random because the teacher picked no favorites. Then ask \emph{could a
fourth-period student have been chosen?} --- \emph{random} does not mean \emph{unplanned}.
Five minutes.

\emph{Row 2, SRS \& Systematic (problems 3--4).} Read the three steps, then run the digit strip
live and \textbf{stop at $951$} --- vote on what to do with it before saying anything; the skip
is the only non-obvious move in an SRS, and it is the label under the display. Work problem~3
yourself; problem~4 is the warm-up with its name --- $k = 15$, start $7$, and \emph{what is
actually random here?} Only the start. Five minutes.

\emph{Row 3, Stratified (problems 5--6).} The warm-up already computed $18$ --- start there
and ask what they were doing. Work problem~5 yourself (one division per grade), hand~6 to the
room, and spend the saved minutes on the word \emph{guarantees}. Four minutes.

\emph{Row 4, Cluster (problems 7--10) --- the crux.} Define cluster and work problem~7
yourself ($2 \times 30 = 60$, and the name). \textbf{Only then let the class read the homeroom
map} in problem~8: both rooms 9th grade, zero seniors. Then \textbf{re-take the hook vote at
problem~9 before you say anything} --- chance chose, nobody counted wrong, and it still
missed. Land the printed callout word by word: a homeroom is alike inside, so it is a stratum;
alike inside means \emph{some from every group}. Problem~10 is the contrast, said aloud before
it is written. Six minutes.

\columnbreak
\textbf{We do (about 15 min).} The Guided Practice row, \emph{Cedar Ridge} (problems 11--14)
--- the same moves the homework will ask alone, on an apartment complex instead of a school.
Read the building layout aloud when you open it: buildings $1$--$6$ studios, $7$--$20$
one-bedrooms, $21$--$30$ two-bedrooms --- problem~13 does not work if students skim past it.
\textbf{Students hold the pen; you hold the questions.} Problem~11 goes on them cold, one plan
at a time. Problem~12 is warm-up arithmetic at $1/10$. \textbf{Problem~13 is the crux
transferred}: the hat gives buildings $2$, $5$, and $6$. Take a hand count on \emph{is this
sample fine?} before anyone writes, and make both sides defend it. Problem~14 is PS.DC.2b ---
the reason, not the word --- and its second question (only a list of buildings) can be said
aloud if time is short.

Circulate during Guided Practice and demand the reason, not the word:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem 11:} ``How many of the groups did you use --- all of them, or a few?''
        The group names are not evidence.
  \item \textbf{Problem 12:} ``You wrote $12$ for the studios. Twelve out of how many?''
  \item \textbf{Problem 13:} ``Chance chose the buildings. So what went wrong?'' A student who
        says \emph{bad luck} has not left the hook vote; the answer is that a building is not
        a mix of the complex.
  \item \textbf{Problem 14:} ``Finish the sentence --- \emph{because the groups are}\,\ldots''
\end{itemize}
While circulating, pick the papers to put up in the debrief --- one problem~13 that named the
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  ...
  \item Put up the two problem~13s you picked. The difference between them is
        not arithmetic --- it is whether the student still believes \emph{random} settles it.
  \item Put problem~2 back up and take the \emph{random-means-unplanned} reflex:
        \emph{``Who could never have been picked? Then who chose?''}
  \item Take problem~14 last, and insist on the form: \emph{stratified, because the groups are
...
(c) runs past a third of the room, open Lesson~1.4 by re-running the \emph{Cluster} row
(problems 7--10)}, and say so plainly.
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{``It was random, so it's fine''} (problem~9, problem~13, HW~5 --- the target
  ...
  \item \textbf{Stratified and cluster swapped} (problem~10, problem~11, HW~5). Probe:
  ...
  \item \textbf{``Random'' meaning ``unplanned''} (problem~2, problem~11, HW~1).
        Probe: \emph{``Who could never have been picked?''} If that list is non-empty, chance
        did not choose.
  \item \textbf{The interval computed as $n \div N$} (problem~4, HW~4): $60 \div 900$ gives
        $0.0\overline{6}$ and the student moves on. Probe: \emph{``What are the units of $k$
        --- names, or fractions?''}
  \item \textbf{``Skip it'' meaning ``stop''} (problem~3, HW~2). The generator keeps
  ...
  \item \textbf{A technique named with no reason} (problem~14, HW~6). PS.DC.2b is the
...
to. Circulate: \textbf{item~5 is the column that tells you whether the \emph{Cluster} row
landed}, and it is
...
re-running problems 7--10 if that third pile ran past a third of the room. Then close
...
and $37$ have already done problem~4; students who write $8$ and $9$ read
``next'' as ``the next name,'' and that misread is fixed at the board in ten seconds before the
notes begin. Item~2's $18$ is the stratified allocation without its name; problem~5 opens on it.
...
definition, mark up the display, and work the first problem of the grid (1, 3, 5, 7); the
class works the rest with the pen in their hand. Rows 1 and~2 must move: five minutes on the
frame and \emph{who chose it}, five on the two list techniques. \textbf{Take the hand count on
problem~2 before anyone writes}; if students write first the trap is gone and the rule becomes
a definition to copy. Run the digit strip live, not read: stop at $951$ and
vote before saying anything --- the skip is the only non-obvious move in an SRS. Problem~6 is
the warm-up with its name; do not re-teach the arithmetic.

\textbf{Spend the time in row~4, \emph{Cluster}}, which carries the misconception. Row~3 is
one division per grade --- four minutes --- and the minute that matters is the word
\emph{guarantees} in problem~6. Then slow down. Define cluster, work problem~7 ($2 \times 30 =
60$), and \textbf{only at problem~8 let the class read that homerooms $1$--$9$ are the 9th
grade}; if the map is on the board before the draw, the crux collapses into a definition. Take
problem~8 from the room --- both rooms 9th grade, zero seniors --- and \textbf{re-take the
hook vote at problem~9 before you say a word}. Let the room
argue; the students who voted \emph{yes} will reach for \emph{bad luck}. Then land the printed
callout: not luck --- \emph{about one draw in four} lands both rooms in one grade, because a
homeroom is alike inside, so it is a stratum. Close the row with problem~10 and the
spoken one-liner: \emph{stratified is some from every group; cluster is everyone from a few}.

\textbf{Guided Practice (11--14) is where the pen changes hands.} \emph{Cedar Ridge} runs the
...
Read the building layout aloud when you open the row. Problem~13 is the crux transferred: a
...
defends it with ``but it was random'' has not left the hook vote. If you only get through 11,
12, 13, and~14's first question, that is the right trade; the building-list question can be
said aloud.

\textbf{There is no independent practice set on this page, and that is deliberate --- the
homework is the individual practice.} Guided Practice is the last row; the period
...
of the room, \textbf{open Lesson~1.4 by re-running problems 7--10}, and say so
plainly.
