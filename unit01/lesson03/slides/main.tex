...
% GRADUAL RELEASE deck: title → targets → warm-up → hook (unresolved) → I DO divider → one or two
% frames per notes ROW (2026-09-12 table shape) → WE DO (a live surface, un-answered) → spoken debrief → close & START THE
...
% ── HOOK — leave it UNRESOLVED; problem 9 (row 4, Cluster) settles it ────────
{
\setbeamercolor{background canvas}{bg=cerulean}
\begin{frame}[plain]
  \vspace{1.2cm}
  \begin{center}
    ...
    box. We settle this at problem 9.}
  \end{center}
\end{frame}
}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 1 $\cdot$ Who Chooses? $\cdot$ problems 1--2}
  \vspace{4pt}
  \begin{columns}[t]
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 2 $\cdot$ SRS \& Systematic $\cdot$ problems 3--4}
  \vspace{2pt}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 3 $\cdot$ Stratified $\cdot$ problems 5--6}
  \vspace{2pt}
  \begin{columns}[t]
...
\begin{frame}[t, plain]
  ...
  \sectionlabel[redacc]{notes row 4 $\cdot$ Cluster $\cdot$ problems 7--10 $\cdot$ the whole point of today}
  \vspace{2pt}
  \begin{itemize}\setlength{\itemsep}{4pt}
    ...
    \item Problem 8 --- rooms 4 and 7: \redink{60 ninth graders. Zero seniors.} Did chance
          choose? \textbf{Yes.} Did anyone count wrong? \textbf{No.}
          \textbf{Problem 9 --- vote again.}
  \end{itemize}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 4 $\cdot$ problem 10 $\cdot$ stratified vs.\ cluster}
  \vspace{0.2cm}
  \begin{center}
    \small
...
\begin{frame}[t, plain]
  ...
  \begin{itemize}\setlength{\itemsep}{4pt}
    \item[11.] Four plans --- name the technique in each.
    \item[12.] The stratified plan: $1/10$ of each unit type. All three rows, then the total.
    \item[13.] The hat gives buildings 2, 5, and 6. Who is in the sample --- and is a building a
          mix of the whole, or a stratum?
    \item[14.] Which plan should management run, \emph{because the groups are}\,\ldots? Then:
          with only a list of buildings, never of residents, which technique still runs?
  \end{itemize}
  \begin{block}{The question behind the question}
    If you defended the cluster draw in 13 with ``but it was random,'' you are still at the hook vote.
  \end{block}
\end{frame}
...
\begin{frame}[t, plain]
  ...
  \begin{enumerate}\setlength{\itemsep}{5pt}
    ...
    \item Problem 2, the third-period class. Who could never have been picked? Then what kind
          of sample is it?
    \item Cedar Ridge, problem 14: the reason, not the word --- \emph{because the groups
          are}\,\ldots
  \end{enumerate}
  \vspace{6pt}
