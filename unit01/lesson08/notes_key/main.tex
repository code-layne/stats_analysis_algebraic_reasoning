%! Project: Design and Conduct a Survey — Unit 1, Lesson 1.8 — Guided Notes & Practice (Answer Key)
...
\pageheader{Unit 1, Lesson 1.8}{Guided Notes \& Practice --- Answer Key}
...
\vtermans{Survey instrument}{The exact wording read to every person in the sample, in order, plus its choices --- the tool that turns a statistical question into data.}
\vtermans{Closed-response item}{A question offering a fixed set of choices, so every answer lands in one category and the answers can be counted.}
\vtermans{Relative frequency}{A category's count divided by the total number of responses, as a decimal or a percent. All the categories' relative frequencies add to 100\%.}
\vtermans{Bar graph}{The display for categorical data: one bar per category, bars separated, height equal to the frequency or the relative frequency.}
\vtermans{Convenience sample}{A sample chosen because it was available --- a class surveying itself, every time. Chance had no turn, so its result describes only itself.}
\end{vocabbox}
...
\par\ansline{Answers vary --- most of the room agrees: ``we matched, so our survey worked.''}
\ansline{(Settled at problem 13: disagree --- chance never chose the class.)}
\end{hookbox}
...
\labelbox{2.2cm}{Formulate}\hspace{0.30cm}\labelbox{2.2cm}{Collect}\hspace{0.30cm}\labelbox{2.2cm}{Represent}\hspace{0.30cm}\labelbox{2.2cm}{Analyze \& report}\par}
\\
& \notesprompt{Run the council's plan --- then look at your own.}
\begin{probgrid}{|Y|Y|}
\pcell{1}{Nobody in the council's survey was assigned to anything --- students were asked, and answers recorded. Observational study or experiment? How do you know?}{1.4cm}{Observational --- nobody was assigned; the council asked and recorded.} & \pcell{2}{Does \emph{``How should the council spend the grant?''} pass the three tests --- answers vary, a group is named, a measurement is named? Repair it if not.}{1.8cm}{No --- no group and no measurement. Repair: ``Which option would the most Riverbend students choose?''} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{3}{Stage 2: the council stratified by grade at $60 \div 900 = 1/15$. How many of the $270$ ninth graders and the $180$ twelfth graders does that call for?}{1.6cm}{$270 \times 1/15 = 18$ ninth graders; $180 \times 1/15 = 12$ twelfth graders.} & \pcell{4}{Your class will ask only the students in this room. Who chose that sample --- chance, or the schedule? What is a sample like that called?}{1.6cm}{The schedule chose it, not chance --- a convenience sample.} \\ \hline
\end{probgrid}
\\ \hline
...
``Don't you agree that our underfunded clubs deserve the grant?''
  & \ans{rule 2} & \ans{leading} \\
``Should the council fund clubs and field trips?''
  & \ans{rule 1} & \ans{double-barrelled} \\
``How old are you: $14$--$16$, $16$--$18$, or $18+$?''
  & \ans{rule 3} & \ans{overlapping} \\
\bottomrule
\end{tabularx}\small
\\
& \notesprompt{A biased instrument is wrong in a direction --- and no arithmetic afterwards can pull it back.}
\begin{probgrid}{|Y|Y|}
\pcell{5}{Repair the leading item so it is neutral. Then say why no arithmetic done later can undo a leading item.}{1.8cm}{``Which one option should the grant fund?'' A leading item bends every answer one way; arithmetic cannot undo that.} & \pcell{6}{Repair the age item so a $16$-year-old has exactly one box to check. Which check on the finished percents would have caught the overlap?}{1.8cm}{``$14$--$15$, $16$--$17$, $18$ or older.'' The $100\%$ check: overlapping boxes push the total past $100$.} \\ \hline
\end{probgrid}
\\ \hline
...
\midrule
Club funding   & $21$ & \ans{$21/60 = 35\%$} \\
Field trips    & $18$ & \ans{$18/60 = 30\%$} \\
Gym equipment  & $15$ & \ans{$15/60 = 25\%$} \\
Longer lunch   & $6$  & \ans{$6/60 = 10\%$} \\
\midrule
...
Tallest bar: \labelbox{2.4cm}{club funding}\quad Gaps because ``choice'' is \labelbox{2.2cm}{categorical}\par}
\\
& \notesprompt{Analyze --- and ask who chose the sample before you multiply.}
\begin{probgrid}{|Y|Y|}
\pcell{8}{About how many of all $900$ students would choose gym equipment? Is the council allowed to say so about the school? Why?}{1.8cm}{$0.25 \times 900 = 225$ students. Yes --- chance chose the $60$, every grade represented.} & \pcell{9}{Last spring's class of $25$: $9$ chose club funding. Compute the percent, then $0.36 \times 900$. \textbf{Vote first:} may the class report that second number about the school?}{1.8cm}{$9/25 = 36\%$; $0.36 \times 900 = 324$. No --- the schedule chose the class, so $324$ describes nobody.} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{10}{Suppose the council had asked $120$ students and $42$ chose club funding. Does the club-funding bar move? What does a bar's height tell a reader --- a count, or a share?}{1.8cm}{$42/120 = 35\%$ --- the bar does not move. A height is a share, not a count.} & \pcell{11}{\textbf{In your own words:} why must the four percents add to $100\%$ --- and which instrument rule makes that happen?}{1.8cm}{Each student picked exactly one box, so the counts add to the total. Rule 3.} \\ \hline
\end{probgrid}
\\ \hline
...
The class's sentence 4 may describe: \labelbox{3.2cm}{this class only}\par}
\\
...
\writespace{1.3cm}{Chance never chose the class, so the match was luck, not method. A different room could have missed by twenty points.}
\\
& \notesprompt{Now settle the hook vote.}
\begin{probgrid}{|Y|Y|}
\pcell{12}{Write the council's sentence 4 --- what it means for the \$$3{,}000$, and about whom.}{1.6cm}{``Club funding leads --- about $315$ of the $900$ students back it.''} & \pcell{13}{\textbf{Vote again:} agree \; disagree. Does matching the council's $35\%$ make the class's $36\%$ trustworthy for all $900$? Explain.}{1.8cm}{Disagree. Chance never chose the class, so the match is luck --- it could have missed by twenty.} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{14}{Write last spring's class's sentence 4. Its subject must be the people the design actually covers.}{1.6cm}{``Of the $25$ students in this class, $36\%$ chose club funding.''} & \pcell{15}{Name one change to the \emph{sample} that would let the class speak about all $900$ --- and one thing that change would cost.}{1.6cm}{Draw a stratified $60$ by chance. It costs class time and a roster.} \\ \hline
\end{probgrid}
\\ \hline
...
Item: \ans{Which one of these should the \$3,000 grant fund?}\par\vspace{5pt}
(A) \ans{Clubs}\ (B) \ans{Field trips}\ (C) \ans{Gym gear}\ (D) \ans{Lunch}}}
\\
...
(D) & & \ans{3} & \ans{$3/25 = 12\%$} \\
\midrule
\textbf{Total} & & \ans{25} & \ans{$100\%$} \\
\bottomrule
\end{tabularx}\small
\\
& \notesprompt{We work these together.}
\begin{probgrid}{|Y|Y|}
\pcell{16}{Name the population, the sample, and $n$. Who chose it --- chance, or the schedule? What kind of sample is that?}{1.6cm}{All $900$ students; the $25$ here, $n = 25$. The schedule chose it --- convenience.} & \pcell{17}{Show the division for our largest category, then write our sentence 4 --- its subject is this class, not the school.}{1.6cm}{$9/25 = 36\%$. ``Of the $25$ in this class, $36\%$ chose club funding.''} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{18}{Whose result may be reported about all $900$ --- the council's $60$, or ours? If ours lands near theirs, what does that prove?}{1.6cm}{Only the council's --- chance chose it. Ours proves nothing; near $35\%$ is luck.} & \pcell{19}{Name the limit you would fix first in our design, and what fixing it would cost.}{1.6cm}{The sample --- draw it by chance from all $900$. Costs a roster and time.} \\ \hline
\end{probgrid}
\\ \hline
\end{guidednotes}
