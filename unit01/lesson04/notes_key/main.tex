...
\vtermans{Bias}{A study is biased when its procedure systematically overestimates or underestimates the population value --- every sample leans the same direction.}
\vtermans{Sampling bias}{Bias from how the sample was chosen: some members of the population are systematically more likely to be picked than others.}
\vtermans{Undercoverage}{Part of the population is left off the sampling frame, so those members can never be chosen at all.}
...
\vtermans{Response bias}{Anything about the survey itself --- who asks, the wording, the order --- that pushes a respondent away from their true answer.}
\end{vocabbox}
...
section~2.
\par\ansline{Answers vary --- most of the room says yes (``more people, better answer'').}
\ansline{(Settled at the end of section 2: no --- 270 of 750 is 36\% again. Same Tuesday, same lean.)}
\end{hookbox}
...
underestimates the population value. Every sample it takes leans the same \ans{direction}.
...
\midrule
At random from the full membership list & $19\%$ & $22\%$ & $19\%$ & \ans{20\%} \\
From whoever arrived Tuesday evening    & $35\%$ & $36\%$ & $37\%$ & \ans{36\%} \\
\bottomrule
\end{tabularx}
\end{center}
...
\ans{Tuesday} row misses the \emph{truth} every single time, and it misses \ans{too high}.
Tuesday evening is lap-swim time, so \ans{lap swimmers} are over-counted. \textit{Consistent
...
\midrule
Ask the members who arrive Tuesday evening. & morning and weekend swimmers & \ans{undercoverage} \\
Email the survey only to members with an address on file. & members with no email on file & \ans{undercoverage} \\
Post the survey on the pool's social media page. & members who do not follow the page & \ans{undercoverage} \\
Mail a card to all $1{,}500$ members; $200$ mail it back. & the $1{,}300$ who never mailed it back & \ans{nonresponse} \\
\bottomrule
\end{tabularx}
\end{center}
...
chosen?} A morning swimmer \ans{could not} have been chosen on Tuesday evening ---
undercoverage. A silent card holder \ans{could} have been --- nonresponse. Both leave
...
\midrule
The lifeguard hands it out and waits; last week's complainers rate the staff ``excellent.'' & \ans{social desirability} \\
A member marks ``no opinion'' on all fifteen questions. & \ans{neutral responding} \\
A member agrees with every statement, even two that contradict each other. & \ans{acquiescence} \\
The survey is titled \emph{``Help us show the pool needs longer hours.''} & \ans{demand} \\
Asked \emph{``Are the lap lanes overcrowded?''} first, $62\%$ say yes to a \$40,000 lane;
asked the money question first, $41\%$ do --- same words, $21$ points apart. & \ans{question order} \\
\bottomrule
\end{tabularx}
\end{center}
...
One Tuesday: $75$ asked, $36\%$, off by \ans{16} points. Ten Tuesdays: $750$ asked,
\ans{36\%}, off by \ans{16} points. Ten times the work, the same answer.
\textbf{Vote again.}

\vspace{3pt}
\begin{center}
\fcolorbox{cerulean}{frost}{\parbox{0.94\linewidth}{\small
\textbf{A larger sample makes an estimate more \ans{precise}, not more \ans{correct}.} Size
shrinks \ans{variability}; nothing about size touches \ans{direction}. The $750$ are still
...
\ans{undercoverage}; calling back the silent removes \ans{nonresponse}; neutral wording and anonymous
...
\ans{undercoverage}. The $450$ households that threw the card away are \ans{nonresponse}. Could one
of them have been chosen? \ans{yes}
...
The mail-in study was off by \ans{24} percentage points --- \ans{144} households
--- and it was \ans{too high}.
...
\par\ansline{More cards from the same procedure: still only the households that chose to answer, and}
\ansline{the car owners answered at twice the rate --- so the 64\% is too high no matter how many cards.}

\vspace{3pt}
\textbf{(e)} The card asked: \emph{``Don't you agree that Cedar Ridge's parking lot is
dangerously overcrowded?''} Name the response bias that wording invites: \ans{acquiescence}. Then
rewrite the question so it pushes neither way.
\par\ansline{``Do you think the amount of parking at Cedar Ridge is too little, about right, or too much?''}
\end{practicebox}
