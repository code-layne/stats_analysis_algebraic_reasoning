...
% GRADUAL RELEASE deck: title → targets → warm-up → hook (unresolved) → I DO divider → one or
% two frames per notes ROW → WE DO (a live surface, un-answered) → spoken debrief → close &
...
% ── HOOK — leave it UNRESOLVED; problem 13 (row 4, Bigger Is Not Better) settles it ──
{
\setbeamercolor{background canvas}{bg=cerulean}
\begin{frame}[plain]
  \vspace{1.2cm}
  \begin{center}
    ...
    box. We settle this at problem 13.}
  \end{center}
\end{frame}
}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 1 $\cdot$ Bias $\cdot$ problems 1--4}
  \vspace{2pt}
  ...
  \end{center}
  \vspace{4pt}
  \begin{block}{Problem 1 --- before you average the rows: which row would you trust?}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 2 $\cdot$ Sampling Bias $\cdot$ problems 5--8}
  \vspace{2pt}
  \begin{columns}[t]
    ...
    \end{column}
    \begin{column}{0.47\textwidth}
      \begin{block}{Nonresponse --- problem 8}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 3 $\cdot$ Response Bias $\cdot$ problems 9--11}
  \vspace{2pt}
  \begin{columns}[t]
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 3 $\cdot$ problem 10 $\cdot$ question order bias}
  \vspace{0.3cm}
  \begin{center}
    \small
...
\begin{frame}[t, plain]
  ...
  \sectionlabel[redacc]{notes row 4 $\cdot$ Bigger Is Not Better $\cdot$ problems 12--15 $\cdot$ the whole point of today}
  \vspace{2pt}
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \textbf{Ten times the work, the same answer. Problem 13 --- vote again.}
...
\begin{frame}[t, plain]
  \ceruleanheader{Guided Practice: Cedar Ridge}
  ...
  \begin{itemize}\setlength{\itemsep}{4pt}
    \item[16.] What share of the $600$ answered? What share of \emph{those} complained? Scale
          that percent to all $600$.
    \item[17.] Every household got a card --- could a silent one have been chosen, so which
          bias? The census says $40\%$: off by how many points, how many households, which way?
    \item[18.] Twice the cards, $64\%$ again; the car owners answered at twice the rate. Why didn't
          more cards help?
    \item[19.] \emph{``Don't you agree the lot is dangerously overcrowded?''} Name the bias; rewrite it neutral.
  \end{itemize}
  \begin{block}{The question behind the question}
    If your answer to 18 is ``they still need more cards,'' you are still at the hook vote.
  \end{block}
\end{frame}
...
\begin{frame}[t, plain]
  ...
  \begin{enumerate}\setlength{\itemsep}{5pt}
    \item Problem 13's ten Tuesdays back on the board. Say the sentence: \emph{$750$ is not
          closer than $75$, because size shrinks variability and this study's problem is direction.}
    \item Problem 8's $200$ cards that came back. Could a silent card holder have been chosen?
          Then which bias is it?
    \item Cedar Ridge, problem 18: the direction and the reason --- \emph{too high, because}\,\ldots
  \end{enumerate}
  \vspace{6pt}
