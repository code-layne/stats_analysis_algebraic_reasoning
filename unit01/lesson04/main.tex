...
\begin{tcolorbox}[colback=frost, colframe=cerulean, boxrule=0.9pt, arc=2mm,
    ...
    members beats $75$. Do not compute, hint at, or resolve the ten-week result until row~4,
    \emph{Bigger Is Not Better} (problems 12--15) --- the hook and rows 1--3 run on the $75$ alone.
\end{tcolorbox}
...
Warm-Up & 5 & Harbor Point's $1{,}500$: the census percent, the Tuesday sample's percent, the
  ...
  $540$ vs.\ $300$ --- off by $240$, too high'' on the board and leave it up --- row~1
  opens on it \\
Guided Notes \& Practice & 35 & Fill the vocabulary box as each term is named; work rows 1--4
  of the table (problems 1--15) with the pen in their hand; then the Guided Practice row,
  \emph{Cedar Ridge} (16--19), together & \textbf{I do / we do, row by row}: read the
  definition, mark up the display, work problems 1, 5, 10, 12; the class works the rest (20)
  $\to$ \textbf{we do} Guided Practice 16--19 (15) \\
Debrief & 10 & Pens down, papers up --- answer aloud, nothing to fill in & Problem~13 back on
  the board; the ``too small'' reflex, then the ``left out means undercoverage''
...
\begin{minipage}[t]{\linewidth}\vspace{0pt}
    ...
    \begin{itemize}[leftmargin=1.1em, itemsep=1pt, topsep=2pt]
      \item \textbf{1.} $300$ of $1{,}500$ as a percent. \textbf{Spirals} to Lesson 1.1's
            relative frequency; \textbf{seeds} the \emph{truth} line --- $20\%$ is the
            parameter every display in the notes is measured against (rows 1 and~4).
      \item \textbf{2.} $27$ of $75$ as a percent. \textbf{Seeds:} the Tuesday-evening
            statistic, $36\%$ --- the study 1.3 threw out, now the biased row of row~1's
            display (problem~1) and the ten-week study's unchanging answer (problem~12).
      \item \textbf{3.} $0.36 \times 1500 = 540$, off by $240$, too high. \textbf{Seeds:} the
            ...
            \emph{direction} of the miss is the whole of row~1 (problems 1--4).
    \end{itemize}
\end{minipage}
&
\begin{minipage}[t]{\linewidth}\vspace{0pt}
    ...
    \begin{itemize}[leftmargin=1.1em, itemsep=1pt, topsep=2pt]
      \item \textbf{Debrief item~3 aloud} and write $20\%$, $36\%$, $540$ vs.\ $300$, \emph{off
            by $240$, too high} on the board. Leave it up all period; row~1 hangs the word
      ...
      \item Item~3(b) is the tell. Students who write $16$ instead of $240$ answered in
            ...
            what problems 12 and~13 want, so credit it and tell them to hold the thought.
...
only that the answer is \textbf{problem~13}, in the \emph{Bigger Is Not Better} row, and that
they will vote again there. The vote is what makes the crux land: students have to have
committed to \emph{bigger is closer} before the notes take it away.
\end{skillbox}
...
diagnostic table (problem~9); the one word bank sits above it.
\begin{multicols}{2}
\textbf{I do / we do, row by row (about 20 min).}

\emph{Row 1, Bias (problems 1--4).} Open on the line already on the board. The two-row display
does the conceptual job visually: both rows wobble, only one is wrong the same way every time.
\textbf{Ask before problem~1 is worked: which row would you trust?} Students reliably pick the
tighter Tuesday row --- ``it agrees with itself'' --- and that wrong answer is worth two
minutes. Work problem~1 yourself ($20\%$ and $36\%$), name \textbf{bias} on the labels, and let
the class do 2--4. \textbf{Problem~4 is the small trap}: \emph{consistent is not correct}.
Five minutes.

\emph{Row 2, Sampling Bias (problems 5--8).} 1.3's frame idea wearing a new name. Read the
definition, mark the picture --- off the frame, chosen but silent --- and work problem~5.
Run 6 and~7 fast and \textbf{save the argument for problem~8}: all $1{,}500$ got a card, the
frame is perfect, and the study is still broken. That is nonresponse, and the test ---
\emph{could that person have been chosen?} --- is the line to make them say aloud. Five minutes.

\emph{Row 3, Response Bias (problems 9--11).} Read the seven-row reference table \emph{once}
--- do not lecture it --- then take problem~9's five rows from the class and let them argue.
Rows 2 and~3 (neutral responding vs.\ acquiescence) are the pair to slow on. Work problem~10
yourself: identical words, $21$ points apart; 11 goes to the room. Six minutes.

\emph{Row 4, Bigger Is Not Better (problems 12--15) --- the crux.} Work problem~12 live,
$270 \div 750 = 36\%$, $16$ points then $16$ points again, and \textbf{re-take the hook vote at
problem~13 before saying a word}. Then land the blue callout: \emph{precise, not correct;
variability, not direction}. Problem~14 attaches each fix to the bias it removes (AFDA.DA.2g);
problem~15 is the counterweight --- a random $75$ beats a biased $750$. Four minutes.

\columnbreak
\textbf{We do (about 15 min).} The Guided Practice row, \emph{Cedar Ridge} (problems 16--19),
alone on page~4 --- the same moves the homework will ask alone, on an apartment complex instead
of a pool. Read the setup aloud when you open it: \emph{every} household got a card ---
problem~17 does not work if students skim past that word. \textbf{Students hold the pen; you
hold the questions.} Problem~16 is warm-up arithmetic with two different denominators on
purpose --- $150/600$ and $96/150$; a student who divides $96$ by $600$ has merged the two
questions. Problem~17 is the frame test transferred, plus scale-and-compare: $384$ against
$240$, $24$ points, too high. \textbf{Problem~18 is the crux transferred}: twice the cards,
$64\%$ again, and the car owners answered at twice the rate. Take a hand count on \emph{did the
extra cards help?} before anyone writes, and make both sides defend it. Problem~19 is
AFDA.DA.2g --- the wording names the bias, and the rewrite has to be genuinely balanced, not
leaning the other way.

Circulate during Guided Practice and demand the reason, not the word:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem 16:} ``Ninety-six out of how many? Who is in that denominator?''
  \item \textbf{Problem 17:} ``Could a silent household have been chosen? Then which bias?''
        Then: ``Too high or too low --- and who is over-counted?''
  \item \textbf{Problem 18:} ``If they mailed to every household ten times, what changes?'' A
        student who says \emph{they'd get closer} has not left the hook vote.
  \item \textbf{Problem 19:} ``Read me your rewrite. Which way does \emph{it} lean?''
\end{itemize}
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  \item \textbf{Put problem~13's ten Tuesdays back up} and make a student say the sentence: $750$ is
  ...
  \item Put problem~8's $200$ returned cards back up and take the
        \emph{left-out-means-undercoverage} reflex: \emph{``Could a silent card holder have
        been chosen? Then which bias is it?''}
  \item Take problem~19 last, and insist on balance: a rewrite that leans the other way
...
re-running problems 12--15}, and say so plainly.
...
would have reported: \emph{the crux head-on, problem~18's move done alone}; \textbf{5} four response biases to name and the
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{``The sample was too small''} (problems 12--15, GP~18, HW~4 --- the target
        ...
        would change?''}
  \item \textbf{Undercoverage and nonresponse fused} (problem~8, GP~17, HW~1 and~6). Both
  ...
  \item \textbf{A bias named with no direction} (problems 2--3, GP~17, HW~3). Probe: \emph{``Too high
        or too low --- and who is over-counted?''} The label alone is half credit.
  \item \textbf{Neutral responding read as extreme, or as acquiescence} (problem~9 rows
        2--3, HW~5). Probe: \emph{``Which boxes did that person actually circle?''}
  \item \textbf{A fix with no bias attached} (problem~14, GP~19, HW~6). ``Ask better
...
re-running problems 12--15 if that third pile ran past a third of the room. Then close
...
period}; row~1 points back at it, and the hook is asked against it. Watch for students who
answer 3(b) with $16$ instead of $240$: they answered in percentage points, not members. Both
are right ideas, and the points form is exactly what problems 12 and~13 will want ---
...
two-row display and the word \emph{bias}, five on undercoverage and nonresponse. \textbf{Ask
which row they would trust before problem~1 is worked}; if the averages go up first, the trap
is gone and \emph{consistent is not correct} becomes a sentence to copy. \textbf{Problem~8 is
the one that argues} --- every member got a card, so the frame is perfect, and the study is
still wrong. If students insist that is the same as undercoverage, use the test question ---
\emph{could that person have been chosen?} --- and hold the line; the Guided Practice's entire
premise is this problem.

\textbf{Spend the time in rows 3 and~4}, which carry the misconception. The seven response
biases are recognition-level, not recall-level: read the reference table once and go to
problem~9's five rows. The pair to slow down on is neutral responding versus acquiescence,
which get fused into ``didn't take it seriously.'' Then the crux. Work problem~12 with the
room, $270 \div 750$, $16$ and $16$ --- and \textbf{re-take the hook vote at problem~13 before
...
\textbf{Guided Practice (16--19) is where the pen changes hands.} \emph{Cedar Ridge} runs the
...
complex instead of a pool. Read the setup aloud and lean on the word \emph{every}.
Problem~16's two fractions have different denominators on purpose; a student who gets $16\%$
divided $96$ by $600$. Problem~18 is the crux transferred: twice the cards, $64\%$ again, and
the direction follows from the car owners answering at twice the rate. A student who defends it
with ``they still need more cards'' has not left the hook vote. Problem~19's rewrite must be
balanced, not flipped --- ``Is there plenty of parking?'' leans the other way. If you only get
through 16--18, that is the right trade; 19 can be said aloud.
...
third of the room, \textbf{open Lesson~1.5 by re-running problems 12--15}, and say
so plainly.

\textbf{Debrief (10 min), spoken.} Problem~13's ten Tuesdays back on the board with a student saying
