...
\begin{enumerate}[leftmargin=1.6em, itemsep=9pt, topsep=3pt]
  ...
  \item The front-desk study.
        \par\vspace{4pt}
        Population: \ans{all 1,200 card holders} \hfill Sample: \ans{the 90 who filled one out}
        \par\vspace{8pt}
        It was a \ans{convenience} sample, and a frame that leaves out part of the population
        causes \ans{undercoverage}. Name one kind of card holder who had almost no chance of being
        included.
        \par\ansline{Anyone who did not visit the library that week --- weekend-only or online-only card holders.}
...
\begin{enumerate}[leftmargin=1.6em, itemsep=2pt, topsep=2pt, start=3]
  ...
  \item A statistician talks her into doing it properly: a random sample of $120$ card holders
        ...
        \textbf{(b)} The front-desk study was too high by \ans{35} points --- about
        \ans{420} card holders.
  ...
  \item Before the phone survey, the director had said: ``The front-desk number is fine, we
        ...
        \par\vspace{2pt}
        Bigger: Plan \ans{A}. \quad Closer to the truth: Plan \ans{B}. \quad
        Plan A's $360$ would report \ans{about 80\%} --- why?
        \par\ansline{Same desk, same visitors, more of them: four weeks of the same procedure still reaches only}
\ansline{the people who come in and bother to write, so the lean is the same --- about 80\%, not 45\%.}
  ...
  \item Name the kind of response bias in each situation.
        ...
        \midrule
        A card holder agrees with all ten statements, two of which conflict. & \ans{acquiescence} \\
        The survey is headed \emph{``Help us win longer weekend hours!''} & \ans{demand} \\
        The story-hour librarian hands out the survey and waits for it. & \ans{social desirability} \\
        A card holder circles the middle box, ``no opinion,'' on every question. & \ans{neutral responding} \\
        \bottomrule
        \end{tabularx}
        \end{center}
        ...
        \ans{21} points --- \ans{question order} bias.

  \item Same phone draw, but only $78$ of the $120$ ever picked up. Percent answered:
        \ans{65\%} \quad The draw was random and every count is correct --- explain why
        the result could still be biased, and name the fix.
        \par\ansline{Nonresponse: the 42 who never picked up may use the library differently from the 78 who}
\ansline{did, so the answering group leans. Fix: keep calling back until every one of the 120 answers.}
