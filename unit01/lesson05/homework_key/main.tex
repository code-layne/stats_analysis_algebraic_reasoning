...
\begin{enumerate}[leftmargin=1.6em, itemsep=9pt, topsep=3pt]
  ...
  \item The director did not put anybody into the program --- card holders signed themselves up.
        ...
        \par\vspace{4pt}
        What kind of study is this? \ans{observational} \hfill Who decided the groups? \ans{the card holders themselves}
        \par\vspace{8pt}
        Explanatory variable: \ans{signed up for the program} \hfill Response variable: \ans{read at least 10 books}
  ...
  \item Compute the reading rate for each group --- watch the two denominators --- then the
        ...
        \par\vspace{2pt}
        The \ans{the program} group is higher by \ans{48} percentage points.
...
\begin{enumerate}[leftmargin=1.6em, itemsep=5pt, topsep=2pt, start=3]
  ...
  \item Same data, two sentences. Decide which one the study supports, and say why.
        ...
        Card holders who signed up read more books than card holders who did not.
          & \ans{yes} & \ans{That is what was seen: $80\%$ vs. $32\%$, a $48$-point gap --- an association.} \\
        The Summer Reading Program more than doubles how much our members read.
          & \ans{no} & \ans{Causal (``doubles''). Nobody was assigned; the readers may have signed up.} \\
        \bottomrule
        \end{tabularx}
        \end{center}
  ...
  \item The director checks one more record: how much each card holder read the
        ...
        \par\vspace{4pt}
        Signed up: \ans{$63/90 = 70\%$} \qquad Did not sign up: \ans{$30/150 = 20\%$}

        \par\vspace{6pt}
        Which of the three stories is this --- and does it make the program look
        \emph{better} or \emph{worse} than it really is?
        \par\ansline{The arrow backwards --- the card holders who already read a lot are the ones who sign up.}
\ansline{It makes the program look better than it is: most of the 48-point gap was there before it.}
  ...
  \item The sample was drawn at random from the full list, every person answered, and every
        ...
        \par\ansline{Bias is about whether the sample looks like the population; it does. Causation is about why}
\ansline{the two groups differ, and the card holders sorted themselves --- no sampling method fixes that.}
