...
% GRADUAL RELEASE deck: title → targets → hook (unresolved) → I DO divider → one frame per
% notes row → WE DO (a live surface, un-answered) → spoken debrief → close & START THE
...
% ── HOOK — leave it UNRESOLVED; problem 14 (row 4, One Parameter) settles it ─
{
\setbeamercolor{background canvas}{bg=cerulean}
\begin{frame}[plain]
  \vspace{1.3cm}
  \begin{center}
    ...
    {\color{frostmid}\small Vote: Ana, Ben, Cleo, or nobody. We settle this at problem 14.}
  \end{center}
\end{frame}
}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 1 $\cdot$ Population \& Sample $\cdot$ problems 1--4}
  \vspace{4pt}
  \begin{columns}[t]
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 2 $\cdot$ Parameter \& Statistic $\cdot$ problems 5--8}
  \vspace{4pt}
  ...
  \end{itemize}
  \vspace{8pt}
  \begin{block}{Problem 8 --- vote first: ``District records show 61\% of Riverbend's 900 students ride a bus.''}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{notes row 3 $\cdot$ Constraints $\cdot$ problems 9--11}
  \vspace{2pt}
  \begin{center}
    \small
...
\begin{frame}[t, plain]
  ...
  \sectionlabel[redacc]{notes row 4 $\cdot$ One Parameter $\cdot$ problems 12--15 $\cdot$ the whole point of today}
  \vspace{4pt}
  \begin{center}
    \small
    \renewcommand{\arraystretch}{1.3}
  ...
  \begin{itemize}\setlength{\itemsep}{4pt}
    \item Three samples. Three statistics. \textbf{Problem 14 --- vote again: who made the mistake?}
...
\begin{frame}[t, plain]
  \ceruleanheader{Guided Practice: Riverbend Jobs}
  ...
  \begin{itemize}\setlength{\itemsep}{5pt}
    \item[16.] Population, sample, $n$ --- and one constraint that kept her from asking all 900.
    \item[17.] The percent, and about how many of the 900 --- then \emph{label} both numbers.
    \item[18.] The assistant principal asks a \emph{different} 50; \textbf{23} have a job.
          Did somebody make a mistake? Which survey found the true percent?
    \item[19.] Her draft: ``\emph{Exactly} 360 students have a job.'' Allowed? Rewrite it.
  \end{itemize}
  \begin{block}{The question behind the question}
    If you picked a survey in 18, you are still at the hook vote.
  \end{block}
\end{frame}

% ── GUIDED PRACTICE problem 19 — shown AFTER the class has worked it ──────────
\begin{frame}[t, plain]
  \ceruleanheader{What a sample lets you say --- and what it does not}
  \sectionlabel[goldacc]{Guided Practice, problem 19 $\cdot$ estimate, not count}
  \vspace{4pt}
