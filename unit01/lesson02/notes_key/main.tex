...
\vtermans{Population}{The entire group the question is about --- every individual you would like the conclusion to describe. Decided by the question, not by who is available.}
\vtermans{Sample}{The part of the population you actually collected data from. The number of individuals in it is the sample size, $n$.}
\vtermans{Parameter}{A number that describes the population. Usually unknown --- and there is exactly one of it, whether or not anyone has ever computed it.}
\vtermans{Statistic}{A number computed from the sample. Known, because you collected the data --- and it changes from one sample to the next.}
\vtermans{Sampling variability}{The change in a statistic from one sample to the next, even though the population did not change. Not a mistake --- it is what samples do.}
\end{vocabbox}
...
\par\ansline{Answers vary --- most of the room picks a volunteer, or says the truth is ``in between.''}
\ansline{(Settled at problem 14: nobody --- three samples, three statistics, one parameter.)}
\end{hookbox}
...
Box: the \labelbox{2.2cm}{population}\quad Oval: the \labelbox{1.8cm}{sample}\quad $n = $ \labelbox{1cm}{50}\par}
\\
& \notesprompt{Name the two groups and the sample size --- \emph{the population first; the sample is chosen out of it.}}
\begin{probgrid}{|Y|Y|}
\pcell{1}{The manager asks $50$ of the market's $800$ Saturday shoppers how they traveled.}{1.6cm}{Population: all 800 Saturday shoppers. Sample: the 50 asked. $n = 50$.} & \pcell{2}{The principal asks $45$ of Riverbend's $900$ students how many hours a week they work.}{1.6cm}{Population: all 900 Riverbend students. Sample: the 45 asked. $n = 45$.} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{3}{A cereal plant weighs $30$ of the $12{,}000$ boxes it filled on Tuesday.}{1.6cm}{Population: the 12,000 boxes filled Tuesday. Sample: the 30 weighed. $n = 30$.} & \pcell{4}{Data from \emph{every} individual is a \textbf{census}. Suppose the manager asked all $800$. Name the population and the sample now.}{2.0cm}{Population: all 800 shoppers. Sample: the same 800 --- the sample is the whole population: a census.} \\ \hline
\end{probgrid}
\\ \hline
...
\end{tikzpicture}\par\vspace{5pt}
The $52\%$ is a \labelbox{2.4cm}{statistic}\qquad the true percent for all $800$ is a \labelbox{2.4cm}{parameter}\par}
...
\begin{probgrid}{|Y|Y|}
\pcell{5}{$52\%$ of the $50$ shoppers asked drove to the market.}{1.2cm}{Describes the 50 asked --- a statistic.} & \pcell{6}{The market had $800$ shoppers on Saturday morning.}{1.6cm}{Describes all 800 shoppers --- a parameter (a count, and still a parameter).} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{7}{The $8$ shoppers asked traveled a mean of $4$ miles.}{1.2cm}{Describes the 8 asked --- a statistic.} & \pcell{8}{District records show $61\%$ of Riverbend's $900$ students ride a bus. \textbf{Vote first, then decide.}}{1.6cm}{Describes all 900 students --- a parameter, even though it is a percent.} \\ \hline
\end{probgrid}
\\ \hline
...
\begin{probgrid}{|Y|Y|}
\pcell{10}{One manager, one clipboard, one morning, $800$ shoppers. Which constraint kept her from a census?}{1.6cm}{Access (or time) --- shoppers leave before she can reach them.} & \pcell{11}{What would she have to do to get the \emph{parameter} itself --- and why is that off the table?}{1.6cm}{Run a census --- ask all 800. Time and access forbid it.} \\ \hline
\end{probgrid}
\\ \hline
...
\writespace{1.4cm}{Each asked a different 50 shoppers, so each got a different statistic. The one parameter never moved.}
\\
& \notesprompt{Now settle the hook vote.}
\begin{probgrid}{|Y|Y|}
\pcell{12}{Compute Ben's and Cleo's statistics the way Ana's was computed: $26/50 = 0.52 = 52\%$.}{1.4cm}{Ben: $24/50 = 0.48 = 48\%$ \par Cleo: $29/50 = 0.58 = 58\%$} & \pcell{13}{How many statistics did the three volunteers produce? How many true percents are there for all $800$ shoppers?}{2.0cm}{Three statistics --- one from each sample. One true percent: there is exactly one parameter for all 800.} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{14}{\textbf{Vote again:} Ana \; Ben \; Cleo \; nobody. Did one of them count wrong? Which one found the true percent for all $800$?}{2.0cm}{Nobody. Nobody counted wrong, and none found the true percent --- three estimates of one parameter.} & \pcell{15}{A fourth volunteer asks $200$ shoppers, not $50$. Would her statistic land closer to the true percent? Exactly on it?}{1.8cm}{Usually closer, but not exactly on it every time --- it is still a statistic.} \\ \hline
\end{probgrid}
\\ \hline
...
\begin{probgrid}{|Y|Y|}
\pcell{16}{Name the population, the sample, and $n$ --- and one constraint that kept her from asking all $900$.}{2.0cm}{Population: Riverbend's 900 students. Sample: the 50 she asked; $n = 50$. Constraint: time (or cost, access).} & \pcell{17}{What percent of those she asked have a job, and about how many of the $900$ is that? Label each number: statistic, parameter, or estimate.}{2.0cm}{$20/50 = 0.40 = 40\%$ --- a statistic. \par $0.40 \times 900 = 360$ --- an estimate of the parameter.} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{18}{One survey says $40\%$; the other $46\%$. Did one make a mistake? Which survey found the true percent for all $900$? Explain.}{2.0cm}{$23/50 = 0.46 = 46\%$. No mistake --- two samples, two statistics. Neither found it: one parameter, both estimate it.} & \pcell{19}{Her draft report: \emph{``Exactly $360$ Riverbend students have a part-time job.''} Allowed? Rewrite it to claim only what her sample allows.}{2.0cm}{No --- 360 was computed from the 40\%, not counted. ``Of the 50 asked, 40\% have a job; that is about 360 of the 900.''} \\ \hline
\end{probgrid}
\\ \hline
\end{guidednotes}
