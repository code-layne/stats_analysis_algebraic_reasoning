...
``Exactly $416$ Saturday shoppers drove to the market.''
  & \ans{No} & \ans{416 is an estimate, not a count} \\
``$52\%$ of Lakeside residents drive to the market.''
  & \ans{No} & \ans{residents were never the population} \\
\bottomrule
\end{tabularx}
\end{center}
\end{notesbox}
...
\par\ansline{No. Neither miscounted --- two different samples of 50 give two different statistics.}
\ansline{Neither found the true percent; there is one parameter, and both 40\% and 46\% estimate it.}

\vspace{3pt}
\textbf{(d)} Name one constraint that kept the principal from asking all $900$ students, and say
which of the five it is.
\par\ansline{Time --- asking all 900 would take class periods nobody can spare (also accept cost or access).}
\end{practicebox}
