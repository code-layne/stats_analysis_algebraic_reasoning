...
\ansline{(Settled at the end of section 2: nobody --- three samples, three statistics, one parameter.)}
\end{hookbox}
...
$52\%$ of the $50$ shoppers asked drove to the market.
  & \ans{sample} & \ans{statistic} \\
The market had $800$ shoppers on Saturday morning.
  & \ans{population} & \ans{parameter} \\
...
\vspace{4pt}
Which kind do you almost always \emph{know}? The \ans{statistic}. \quad Which kind do you actually \emph{want}? The \ans{parameter}. You use the number you have to \textbf{estimate} the number you want --- that gap is the whole reason this course exists.
...
percent for all $800$? \ans{none of them}
...
\ans{stays put}, because the $800$ shoppers did not change. Each statistic is an \textbf{estimate} of the one parameter; a larger sample usually lands \ans{closer} to it, but none lands exactly on it every time.}}
\end{center}
...
\textit{``Of the \ans{50} students we asked, \ans{40}\% have a part-time job. If our
sample represents the school, that is about \ans{360} of Riverbend's \ans{900}
students.''}

\vspace{2pt}
Her draft says instead: \textit{``Exactly $360$ Riverbend students have a part-time job.''}
Allowed? \ans{No} \quad Why not?
\par\ansline{360 was computed from the 40\%, not counted --- it is an estimate of the parameter.}
\end{practicebox}
