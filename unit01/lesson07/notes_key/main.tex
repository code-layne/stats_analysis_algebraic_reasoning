%! Comparing Studies and Choosing a Method — Unit 1, Lesson 1.7 — Guided Notes (Answer Key)
...
\vocabans{Controlled experiment}{A study in which the researcher assigns the
units to groups by chance and imposes a treatment on at least one group, holding
everything else the same. It can support a cause-and-effect conclusion.}
\vocabans{Observational study}{A study in which the researcher measures or
surveys individuals without assigning anyone to a group. The groups already
exist, so the study can report an association only.}
\vocabans{Survey}{The same fixed set of questions given to every individual in a
chosen sample. Best for collecting counts, percents, and attitudes from many
people at once.}
\vocabans{Interview}{A one-on-one conversation built on planned questions,
usually recorded. Best when you need depth --- the reasons behind an answer, in
the person's own words.}
\vocabans{Focus group}{A small-group discussion led by a facilitator, where
members hear and build on each other's answers. Best when the group's shared
experience says more than any one person's.}
\vocabans{Content analysis}{A study of records or documents that already exist
--- borrowing histories, minutes, transcripts, past test scores. Best for
...
\ansline{No. She would have to assign students a $45$-minute bus ride --- which means}
\ansline{assigning where their families live. Nobody can do that, so she must watch instead.}
\end{hookbox}
...
Who decides which group each individual is in?
  ...
  & \ans{Chance does --- the researcher runs a generator and assigns each unit.} \\
Does the researcher impose a treatment?
  & No --- the researcher only measures or asks.
  & \ans{Yes --- at least one group is given the treatment.} \\
Are the two groups alike before the study starts?
  & \ans{Not necessarily. They built themselves, so they may differ in many ways.}
  & \ans{Yes --- random assignment spreads every other variable evenly.} \\
Strongest conclusion it earns
  & \ans{An association only.}
  & \ans{Cause and effect --- the treatment caused the difference.} \\
\bottomrule
\end{tabularx}
\end{center}
...
Watched $150$ records (1.5) & $65\%$ of users vs.\ $80\%$ of non-users
  & \ans{Using the Study Center is associated with a lower pass rate.} \\
Assigned $120$ volunteers (1.6) & $75\%$ treated vs.\ $55\%$ control
  & \ans{Attending the Study Center caused a higher pass rate.} \\
\bottomrule
\end{tabularx}
\end{center}
...
Does a $10$-minute silent reading block raise reading scores?
  & \ans{Yes} & \ans{Nothing stops her --- run the experiment.} \\
Do students who smoke have worse lung function at $30$?
  & \ans{No} & \ans{Unethical --- nobody may be assigned to smoke.
    Observational.} \\
Do retired pool members sleep more than members who work full time?
  & \ans{No} & \ans{Impossible --- retirement cannot be handed out.
    Observational.} \\
Do students who ride the bus over $45$ minutes score lower?
  & \ans{No} & \ans{Impossible --- you cannot assign where a family lives.
    Observational.} \\
\bottomrule
\end{tabularx}
\end{center}
...
a(n) \ans{association} but never a \ans{cause-and-effect claim}.
...
Collect & Who are the units, and how does \emph{chance} put them in groups?
  & \ans{A generator sends half the volunteers to a weekly check-in and half
    to none.} \\
Represent & How will we organize what comes back?
  & \ans{A two-row table --- each group's count and percent turning in every
    assignment.} \\
Analyze \& report & What do the numbers say, \emph{in context}?
  & \ans{Compare the two percents, find the gap, and say it in context.} \\
\bottomrule
\end{tabularx}
\end{center}
...
what does it do? \ans{Collect --- it \emph{assigns} the units by chance.}
...
read, so the method was \ans{observation} --- and the study was still
a(n) \ans{experiment}, because chance assigned the groups.
...
Does a weekly recycling reminder increase how much a household recycles?
  & \ans{Experiment} & \ans{Observation --- weigh each bin} \\
Do households in the renovated building recycle more than households in the old
building? & \ans{Observational} & \ans{Content analysis of the collection
  records} \\
Why do some households never use the recycling bins at all?
  & \ans{Observational} & \ans{Interviews} \\
\bottomrule
\end{tabularx}
\end{center}
...
\ansline{Living in the renovated building is associated with recycling more --- nothing stronger.}
\ansline{The renovated building also got new bins, and its households may be newer tenants.}
\end{practicebox}
