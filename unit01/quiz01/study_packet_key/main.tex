%! Unit 1: Asking Questions with Data: Study Design — Quiz 1 Study Packet (Lessons 1.0–1.3) — blank (Answer Key)
...
\pageheader{Unit 1: Asking Questions with Data: Study Design}{Quiz 1 Study Packet --- Lessons 1.0--1.3 --- Answer Key}
\namedateperiod
...
\vspace{2pt}
(a) Who are the \textbf{individuals} in this data set? \ans{The 480 Ashford students}
...
Minutes spent reading last night & \ans{Quantitative} \\
Locker number & \ans{Categorical} \\
Plays a sport (yes / no) & \ans{Categorical} \\
\end{tabularx}
...
\par\ansline{Arithmetic on a locker number means nothing --- a ``mean locker number'' describes}
\par\ansline{no student. The digits are labels, so locker number is categorical.}
...
\par\ansline{Of the 40 Ashford students surveyed, 18 --- or $45\%$ --- ride the bus to school,}
\par\ansline{more than any other method.}
...
\par\ansline{Stage 2, collect data. Students who do not ride the bus were never in the bus line,}
\par\ansline{so they could never be chosen --- the flaw entered there, not at stage 4.}
...
\vspace{2pt}
(a) Population: \ans{All 1,000 cardholders} \quad Sample size: \ans{$40$}

\vspace{3pt}
Sample: \ans{The 40 cardholders who were surveyed}
...
\par\ansline{A statistic --- it describes the 40 cardholders surveyed, not all 1,000.}

(d) Name one \textbf{constraint} that keeps the director from taking a census of all $1{,}000$
cardholders.

\par\ansline{Time --- surveying every one of 1,000 cardholders would take far too long.}
...
Number sampled & \ans{$15$} & \ans{$13$} & \ans{$12$} & \ans{$40$} \\
\end{tabularx}
...
\vspace{2pt}
\ans{$5$} \quad \ans{$17$} \quad \ans{$29$} \quad \ans{$41$}
...
\par\ansline{The homerooms are grouped by grade, so a homeroom is alike inside --- a stratum, not}
\par\ansline{a mix of the whole. Two homerooms would be one or two grades and miss the rest.}
...
\midrule
Put all $480$ student numbers in a generator and drew $40$. & \ans{Simple random} \\
Split the school by grade and drew some students at random from every grade, in proportion. &
  \ans{Stratified} \\
Stood by the front door before school and asked whoever walked in. & \ans{Convenience} \\
Ordered the list and took every $12$th student after a random start. & \ans{Systematic} \\
\end{tabularx}
