...
\pageheader{Unit 1: Asking Questions with Data: Study Design}{Quiz 1 --- Lessons 1.0--1.3 --- Answer Key}
\namedateperiod
...
\begin{enumerate}[leftmargin=1.9em, itemsep=3.5pt, topsep=1pt]
  \item \ans{G} Individual
  \item \ans{C} Quantitative variable
  \item \ans{E} Statistical question
  \item \ans{B} Sample
  \item \ans{H} Parameter
  \item \ans{A} Statistic
  \item \ans{D} Sampling frame
  \item \ans{F} Stratified sample
\end{enumerate}
\end{minipage}\hfill
...
\vspace{2pt}
(a) Who are the \textbf{individuals} in this data set? \ans{The 840 Westbrook students}
...
\midrule
Locker number & \ans{Categorical} \\
Minutes at practice yesterday & \ans{Quantitative} \\
Takes the bus (yes / no) & \ans{Categorical} \\
Preferred field-trip destination & \ans{Categorical} \\
\end{tabularx}
...
\par\ansline{``How many minutes is practice for Westbrook students who play a sport?''}
...
\midrule
Science museum & $22$ & \ans{$44\%$} \\
Aquarium       & $14$ & \ans{$28\%$} \\
State capitol  & $9$  & \ans{$18\%$} \\
Theater        & $5$  & \ans{$10\%$} \\
\end{tabularx}
...
\par\ansline{Of the 50 Westbrook students surveyed, 22 --- or $44\%$ --- chose the science museum,}
\par\ansline{more than any other destination.}
...
\par\ansline{$44\%$ is not ``most'' --- most means more than half. And the survey covers the 50}
\par\ansline{students asked, not all 840, so the sentence reaches beyond its own group.}
...
\vspace{2pt}
(a) Population: \ans{All 1,400 members} \quad Sample size: \ans{$70$}

\vspace{3pt}
Sample: \ans{The 70 members who were surveyed}
...
\par\ansline{A statistic --- it describes the 70-member sample, not all 1,400 members.}

(d) Name one \textbf{constraint} that keeps the manager from taking a census of all $1{,}400$
members.

\par\ansline{Time --- reaching every one of 1,400 members would take far too long.}
...
\midrule
Put all $840$ student numbers in a generator and drew $42$. & \ans{Simple random} \\
Split the school by grade and drew students at random from every grade, in proportion. &
  \ans{Stratified} \\
Ordered the list and took every $20$th student after a random start. & \ans{Systematic} \\
Drew $2$ homerooms at random and surveyed everyone in them. & \ans{Cluster} \\
\end{tabularx}
...
\par\ansline{$926$ --- it is off the frame of $001$--$840$, so skip it and read the next number.}
...
\par\ansline{Stratified. The grades differ a lot and a numbered list of all 840 exists, so draw}
...
\par\ansline{Homerooms are built by grade, so each one is alike inside --- a stratum, not a mix}
\par\ansline{of the whole. Her 56 students are one or two grades, so the $75\%$ estimates}
\par\ansline{nothing about all 840. She may only say $75\%$ of those 56 students said yes.}

\vspace{2pt}
(d) Is her $75\%$ a parameter or a statistic? Name one change to her method that would make it
an estimate worth trusting.

\par\ansline{A statistic. Draw the sample at random from the whole numbered list of 840, or}
