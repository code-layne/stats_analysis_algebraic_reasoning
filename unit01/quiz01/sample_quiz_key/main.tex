%! Unit 1: Asking Questions with Data: Study Design — Quiz 1 — Sample Quiz (Lessons 1.0–1.3) — blank (Answer Key)
...
\pageheader{Unit 1: Asking Questions with Data: Study Design}{Quiz 1 --- Sample Quiz --- Lessons 1.0--1.3 --- Answer Key}
\namedateperiod
...
\begin{enumerate}[leftmargin=1.9em, itemsep=3.5pt, topsep=1pt]
  \item \ans{D} Individual
  \item \ans{G} Categorical variable
  \item \ans{A} Statistical question
  \item \ans{F} Population
  \item \ans{B} Parameter
  \item \ans{H} Statistic
  \item \ans{C} Sampling frame
  \item \ans{E} Cluster sample
\end{enumerate}
\end{minipage}\hfill
...
\vspace{2pt}
(a) Who are the \textbf{individuals} in this data set? \ans{The 720 Millbrook students}
...
\midrule
Advisory group number & \ans{Categorical} \\
Minutes on the bus this morning & \ans{Quantitative} \\
Buys lunch (yes / no) & \ans{Categorical} \\
Preferred spirit-week theme & \ans{Categorical} \\
\end{tabularx}
...
\par\ansline{It names no group and no measurement, so it has one answer, not answers that vary.}
\par\ansline{``How many minutes is the bus ride for Millbrook students who ride the bus?''}
...
\midrule
Decades     & $24$ & \ans{$40\%$} \\
Color wars  & $18$ & \ans{$30\%$} \\
Sports day  & $12$ & \ans{$20\%$} \\
Movie night & $6$  & \ans{$10\%$} \\
\end{tabularx}
...
\par\ansline{Of the 60 Millbrook students surveyed, 24 --- or $40\%$ --- chose a decades theme,}
\par\ansline{more than any other theme.}
...
\par\ansline{$40\%$ is not ``most'' --- most means more than half. And the survey covers the 60}
\par\ansline{students asked, not all 720, so the sentence reaches beyond its own group.}
...
\par\ansline{A statistic --- it describes the 80-member sample, not all 1,200 members.}

(d) Name one \textbf{constraint} that keeps the manager from taking a census of all $1{,}200$
members.

\par\ansline{Time --- reaching every one of 1,200 members would take far too long.}
...
\midrule
Put all $720$ student numbers in a generator and drew $48$. & \ans{Simple random} \\
Split the school by grade and drew students at random from every grade, in proportion. &
  \ans{Stratified} \\
Ordered the list and took every $15$th student after a random start. & \ans{Systematic} \\
Drew $2$ advisory groups at random and surveyed everyone in them. & \ans{Cluster} \\
\end{tabularx}
...
\vspace{-2pt}
\ans{$7$} \quad \ans{$22$} \quad \ans{$37$} \quad \ans{$52$}
...
\par\ansline{$839$ --- it is off the frame of $001$--$720$, so skip it and read the next number.}
...
\par\ansline{Stratified. The grades differ a lot and a numbered list of all 720 exists, so draw}
\par\ansline{students at random from every grade, in proportion to grade size.}
...
\par\ansline{Advisory groups are built by grade, so each one is alike inside --- a stratum, not a}
\par\ansline{mix of the whole. Her 48 students are one or two grades, so the $62.5\%$ estimates}
\par\ansline{nothing about all 720. She may only say $62.5\%$ of those 48 students said yes.}

\vspace{2pt}
(d) Is her $62.5\%$ a parameter or a statistic? Name one change to her method that would make
it an estimate worth trusting.

\par\ansline{A statistic. Draw the sample at random from the whole numbered list of 720, or}
\par\ansline{stratify by grade, so every grade can be chosen.}
