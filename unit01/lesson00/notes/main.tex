%! Unit Launch: Study Design — Unit 1, Lesson 1.0 — Guided Notes & Practice
...
\vterm{Individual}
\vterm{Variable}
\vterm{Categorical variable}
\vterm{Quantitative variable}
\end{vocabbox}
...
A is the \labelbox{1.5cm}{}\qquad B is the \labelbox{1.5cm}{}\qquad C is the \labelbox{1.5cm}{}\par}
\\
& \notesprompt{Turn a bare number into data --- name the who, the what, and the units.}
\begin{probgrid}{|Y|Y|}
\pcell{1}{Warm-up item 3 gave you $14$. Write it as data: a sentence with a who, a what, and units.}{1.6cm}{} & \pcell{2}{The blog's $14$ has a who (six players) and a what (jersey number). What are its \emph{units}? Is $14$ an amount of anything?}{1.6cm}{} \\ \hline
\end{probgrid}
\\ \hline
...
\end{tikzpicture}\par\vspace{5pt}
The shaded row is one \labelbox{2.0cm}{}\qquad the shaded column is one \labelbox{2.0cm}{}\par}
\\
& \notesprompt{Read the roster. Count, and say what you counted.}
\begin{probgrid}{|Y|Y|}
\pcell{3}{How many individuals does the roster describe? How many variables does it record? (Is ``Player'' one of them?)}{1.6cm}{} & \pcell{4}{The athletic director adds a column, \emph{Free throws made}, and a seventh player, Jo. Individuals now? Variables now?}{1.6cm}{} \\ \hline
\end{probgrid}
\\ \hline
...
\end{tikzpicture}\par\vspace{5pt}
Height (in) is \labelbox{2.8cm}{}\qquad Captain is \labelbox{2.8cm}{}\par}
\\
...
\midrule
Position & \blank{2.9cm} & Points      & \blank{2.9cm} \\
Captain  & \blank{2.9cm} & Height (in) & \blank{2.9cm} \\
\bottomrule
\end{tabularx}\small
\\
& \notesprompt{Summarize each kind the way its type allows --- and say how you decided, not ``it's words'' or ``it's numbers.''}
\begin{probgrid}{|Y|Y|}
\pcell{6}{A mean is not available for Captain. Summarize it the other way: what percent of the six players are captains?}{1.4cm}{} & \pcell{7}{Find the mean of Points, then say it as data --- who, what, units.}{1.6cm}{} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{8}{A survey records each student's number of siblings: $2, 0, 3, 1, 2, 4$. Which type? How do you know?}{1.4cm}{} & \pcell{9}{\textbf{Jersey \#. Vote first:} categorical or quantitative? Record the class split, then your reason. \textbf{We settle it at problem~10.}}{1.6cm}{} \\ \hline
\end{probgrid}
\\ \hline
...
\begin{probgrid}{|Y|Y|}
\pcell{10}{\textbf{Vote again:} is the blog's conclusion correct? \; yes \; no. What type is Jersey \#, and what does the $14$ tell you about the team?}{1.7cm}{} & \pcell{11}{A school lists grade levels $9, 10, 11, 12$; the mean is $42/4 = 10.5$. Who is in grade $10.5$? What type is Grade level?}{1.4cm}{} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{12}{Two ZIP codes, $22030$ and $22201$, have a mean of $22115.5$. What place is that? What type is ZIP code?}{1.2cm}{} & \pcell{13}{Height (in) is digits too: $420/6 = 70$. Who or what does $70$ inches describe? What type is Height?}{1.3cm}{} \\ \hline
\end{probgrid}
\\ \hline
...
\begin{probgrid}{|Y|Y|}
\pcell{14}{How many individuals does the table describe, and how many variables? Name one of each.}{1.4cm}{} & \pcell{15}{Classify Commute, Lunch, and Grade. Then run the digit test on Grade ($63/6 = 10.5$) and fix Theo's reason.}{1.8cm}{} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{16}{Find the mean commute, then say it as data --- who, what, units.}{1.4cm}{} & \pcell{17}{\emph{How long is Priya's commute?} \emph{How long are Riverbend students' commutes?} Which takes data? Rewrite the other.}{1.6cm}{} \\ \hline
\end{probgrid}
\\ \hline
\end{guidednotes}
